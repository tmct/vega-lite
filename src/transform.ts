...
export interface RegressionTransform {
  ...
  /**
   * The data field of per-row weights for weighted least squares regression. Only supported when `method` is `"linear"` or `"constant"`; specifying a weight with any other method is an error.
   */
  weight?: FieldName;
...
export interface CorrelationTransform {
  ...
  /**
   * The data field of per-row weights. When omitted, each row is treated as having weight 1 and the output is the unweighted Pearson correlation.
   */
  weight?: FieldName;
   ...
   *
   * __Default value:__ `["corr"]`
   */
  as?: [FieldName];
}

export function isCorrelation(t: Transform): t is CorrelationTransform {
  return hasProperty(t, 'correlation');
}
